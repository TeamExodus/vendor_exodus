...
page = 1
while not depsonly:
    result = json.loads(urllib.request.urlopen("https://api.github.com/users/TeamExodus/repos?page=%d" % page).read().decode())
    if len(result) == 0:
        break
    for res in result:
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if exists_in_tree(lm, repo_name):
            print('TeamExodus/%s already exists' % (repo_name))
            continue

        print('Adding dependency: TeamExodus/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "TeamExodus/%s" % repo_name, "revision": "n" })
...
def fetch_dependencies(repo_path, fallback_branch = None):
    ...
    dependencies_path = repo_path + '/exodus.dependencies'
    syncable_repos = []
...
print("Repository for %s not found in the TeamExodus Github repository list. If this is in error, you may need to manually add it to your local_manifests/redefined_manifest.xml." % device)
